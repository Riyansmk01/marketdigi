...
function RegisterForm() {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    if (!otpCode || otpCode.length < 4) {
      setErrorMsg('Masukkan kode verifikasi WhatsApp yang valid')
      return
    }

    // Real Seller Registration via Supabase
    try {
      const { data, error } = await supabase.auth.signUp({
        email,
        password,
        ...
        {step === 1 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', marginBottom: '2rem' }}>
                ...
                {role === 'seller' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', borderTop: '1px solid var(--glass-border)', paddingTop: '1.5rem', marginTop: '0.5rem' }}>
                    ...
                    <Input 
                      label="No. WhatsApp (Untuk Verifikasi)" 
                      type="tel" 
               ...
               <Button type="submit" variant="primary" size="lg" style={{ width: '100%' }} className="btn-3d">
                {role === 'seller' ? 'Lanjut Verifikasi WhatsApp' : 'Daftar Sekarang'}
              </Button>
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              ...
              <h2 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '0.5rem' }}>Verifikasi WhatsApp</h2>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                Masukkan 4 digit kode verifikasi yang baru saja kami kirimkan ke nomor WhatsApp <strong>{phone}</strong> Anda.
              </p>
            </div>
            ...
            <form onSubmit={handleVerifyOTP}>
              ...
              <div style={{ textAlign: 'center', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                Tidak menerima kode? <button type="button" onClick={() => toast.info('OTP Terkirim Ulang!')} style={{ background: 'transparent', border: 'none', color: 'var(--accent-color)', cursor: 'pointer', fontWeight: 'bold' }}>Kirim Ulang</button>
              </div>
            </form>
          </>
        )}
